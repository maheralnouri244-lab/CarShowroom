...
void Garden::drawFence(float w, float l) {
    ...
    for (float i = -w / 2; i <= w / 2; i += gap) {
        glPushMatrix(); 
        glTranslatef(i, poleH / 2, -l / 2); glScalef(1, poleH, 1); 
        glutSolidCube(1);
        glPopMatrix();
...
void Garden::draw(float x, float y, float z, bool isNight) {
    ...
    float gardenW = 350.0f;
    float gardenL = 500.0f;
    float lakeW = 200.0f;
    float lakeL = 300.0f;
    ...
    glBegin(GL_QUADS); 
    glNormal3f(0, 1, 0);
    float topH = (gardenL - lakeL) / 2.0f; 
    float topZ = -lakeL / 2 - topH / 2;
    glTexCoord2f(0, 0);
    glVertex3f(-gardenW / 2, 0.05f, topZ - topH / 2);
    glTexCoord2f(1, 0);
    glVertex3f(gardenW / 2, 0.05f, topZ - topH / 2);
    glTexCoord2f(1, 1);
    glVertex3f(gardenW / 2, 0.05f, topZ + topH / 2);
    ...
    glTexCoord2f(0, 0); 
    glVertex3f(-gardenW / 2, 0.05f, botZ - topH / 2); 
    glTexCoord2f(1, 0);
    glVertex3f(gardenW / 2, 0.05f, botZ - topH / 2);
    glTexCoord2f(1, 1);
    glVertex3f(gardenW / 2, 0.05f, botZ + topH / 2); 
    ...
    glTexCoord2f(0, 0); 
    glVertex3f(-lakeW / 2 - sideW, 0.05f, -lakeL / 2); 

    glTexCoord2f(1, 0); 
    glVertex3f(-lakeW / 2, 0.05f, -lakeL / 2);
