...
const float CAR_PALETTE[4][4] = {
    { 0.1f, 0.2f, 0.7f, 1.0f }, // 0: Blue
    { 0.7f, 0.1f, 0.1f, 1.0f }, // 1: Red
    { 0.2f, 0.6f, 0.2f, 1.0f }, // 2: Green
    { 0.8f, 0.7f, 0.1f, 1.0f }  // 3: Yellow
};

// باقي الألوان الثابتة
...
const float COL_GLASS[] = { 0.2f, 0.3f, 0.5f, 0.6f };

BeetleCar::BeetleCar() {
    currentDoorAngle = 0.0f;
    targetDoorAngle = 0.0f;
    currentBodyColor = CAR_PALETTE[0]; // Default Blue
}
void BeetleCar::toggleDoors() {
    if (targetDoorAngle < 1.0f) targetDoorAngle = 60.0f;
    else targetDoorAngle = 0.0f;
}

void BeetleCar::updateAnimation() {
    if (currentDoorAngle < targetDoorAngle) {
        currentDoorAngle += 2.0f;
        if (currentDoorAngle > targetDoorAngle) currentDoorAngle = targetDoorAngle;
    }
...
void BeetleCar::drawWhitewallWheel() {
    float r_tire = 0.38f, w_tire = 0.28f, r_rim = 0.23f;
    glPushMatrix(); glRotatef(90, 0, 1, 0);
    setMaterial(COL_TIRE, 0.1f);
    glPushMatrix(); glTranslatef(0, 0, -w_tire / 2); drawCylinder(r_tire - 0.02f, w_tire, 24); glPopMatrix();
    for (int i = 0;i < 40;i++) { glPushMatrix(); glRotatef(i * 9, 0, 0, 1); glTranslatef(r_tire, 0, 0); glScalef(0.04f, 0.02f, w_tire * 0.95f); glutSolidCube(1.0f); glPopMatrix(); }
    glPushMatrix(); glTranslatef(0, 0, -w_tire / 2 + 0.02f); glutSolidTorus(0.04, r_tire - 0.04, 8, 30); glPopMatrix();
    glPushMatrix(); glTranslatef(0, 0, w_tire / 2 - 0.02f); glutSolidTorus(0.04, r_tire - 0.04, 8, 30); glPopMatrix();
    setMaterial(COL_WHITE, 0.05f); glPushMatrix(); glTranslatef(0, 0, w_tire / 2 - 0.015f); glutSolidTorus(0.025, 0.30, 8, 40); glPopMatrix();
    setMaterial(COL_CHROME, 0.9f); glPushMatrix(); glTranslatef(0, 0, w_tire / 2 - 0.06f); glutSolidTorus(0.03, r_rim, 8, 30); glPopMatrix();
    for (int i = 0;i < 10;i++) { glPushMatrix(); glTranslatef(0, 0, w_tire / 2 - 0.08f); glRotatef(i * 36, 0, 0, 1); glTranslatef(0, r_rim * 0.5f, 0); glScalef(0.04f, 0.5f, 0.02f); glutSolidCube(1.0f); glPopMatrix(); }
    glPushMatrix(); glTranslatef(0, 0, w_tire / 2 - 0.06f); drawDisk(0.08f, 16); glPopMatrix(); glPopMatrix();
}
...
void BeetleCar::drawExtendedChassis() {
    ...
    for (int i = -1; i <= 1; i += 2) {
        glPushMatrix(); glTranslatef(i * 0.65f, 0.5f, zF); glRotatef(90, 0, 1, 0); glScalef(1.2f, 0.65f, 0.5f); glutSolidSphere(0.6f, 20, 20); glPopMatrix();
...
void BeetleCar::drawGlass() {
    float y = 0.7f, z = 1.0f; setMaterial(COL_GLASS);
    glPushMatrix(); glTranslatef(0, y + 0.35f, z); glRotatef(-30, 1, 0, 0); glScalef(1.5f, 0.8f, 0.05f); glutSolidCube(1.0f); glPopMatrix();
}

void BeetleCar::draw(float x, float y, float z, float scale, float rotateY, int colorIndex) {
    ...
    glPushMatrix(); glTranslatef(-sX, 0.3f, hF); glRotatef(currentDoorAngle, 0, 1, 0); drawGenericDoor(false, true); glPopMatrix();
    glPushMatrix(); glTranslatef(-sX, 0.3f, hR); glRotatef(currentDoorAngle, 0, 1, 0); drawGenericDoor(false, false); glPopMatrix();
    glPushMatrix(); glTranslatef(sX, 0.3f, hF); glRotatef(-currentDoorAngle, 0, 1, 0); drawGenericDoor(true, true); glPopMatrix();
    glPushMatrix(); glTranslatef(sX, 0.3f, hR); glRotatef(-currentDoorAngle, 0, 1, 0); drawGenericDoor(true, false); glPopMatrix();
    drawGlass();
    glPopMatrix();
}
